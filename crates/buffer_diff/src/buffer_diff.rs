...
impl BufferDiffInner {
    ...
    fn stage_or_unstage_hunks(
        ...
        } in hunks.iter().cloned()
        {
            ...
            // TODO: `while` or `if`
            while let Some(preceding_pending_hunk_item) = old_pending_hunks.item() {
                if preceding_pending_hunk_item
                    .buffer_range
        ...
        // - trigger staging operation
        // - try adding it to the index
        // - meanwhile, we trigger another staging operation
        // - generate new text??
        //   - try adding it to the index again
        //
        // conclusion: we don't to worry about failures

        let buffer_range_iter = pending_hunks.iter().cloned().collect::<Vec<_>>();

        assert!(
            buffer_range_iter.windows(2).all(|a| a[0]
                .buffer_range
                .start
                .cmp(&a[1].buffer_range.start, buffer)
                .is_lt()),
            "{buffer_range_iter:#?}"
        );
